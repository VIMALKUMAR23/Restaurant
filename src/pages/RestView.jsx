...
function RestView() {
  const {id}= useParams()
  console.log("get id by useParams");
  console.log(id);
  ...
  return (
    <>
      <Row className="mt-5 mb-5">
        ...
        <Col ms={7} className="px-5">
          <hr />
          <h4 className="text-center">
            Restaurant <span className="text-warnng">Details</span>
          </h4>
          <hr />
          <ListGroup>
            <ListGroup.Item>
              <h5 className="text-center p-2">Restaurant: {selectedRestaurant?.name}</h5>{" "}
            </ListGroup.Item>
            <ListGroup.Item>Neighborhood: {selectedRestaurant?.neighborhood}</ListGroup.Item>
            <ListGroup.Item>Addess: {selectedRestaurant?.address}</ListGroup.Item>
            <ListGroup.Item>Cuisin Type: {selectedRestaurant?.cuisine_type}</ListGroup.Item>
            <ListGroup.Item className="text-center p-3 mb-3">
              ...
              <button className="btn btn-warning ms-3 mt-2" onClick={() => setOpen(!open)}>
